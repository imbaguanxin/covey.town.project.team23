...
export default function TownSelection({ doLogin }: TownSelectionProps): JSX.Element {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      let privateMessage = <></>;
      if (!newTownIsPublic) {
        privateMessage = <p>This town will NOT be publicly listed. To re-enter it, you will need to use this ID: {newTownInfo.coveyTownID}</p>;
      }
      toast({
            ...
            <Flex p='4'>
              <Box flex='1'>
                <FormControl>
                  ...
                  <Input name='townName' placeholder='New Town Name' value={newTownName} onChange={event => setNewTownName(event.target.value)} />
                </FormControl>
              </Box>
              <Box>
                <FormControl>
            ...
            <Box borderWidth='1px' borderRadius='lg'>
              <Flex p='4'>
                <FormControl>
                  ...
                  <Input name='townIDToJoin' placeholder='ID of town to join, or select from list' value={townIDToJoin} onChange={event => setTownIDToJoin(event.target.value)} />
                </FormControl>
                  ...
                  {currentPublicTowns?.map(town => (
                    <Tr key={town.coveyTownID}>
                      ...
                      <Td role='cell'>
                        ...
                        <Button onClick={() => handleJoin(town.coveyTownID)} disabled={town.currentOccupancy >= town.maximumOccupancy}>
                          Connect
                        </Button>
                      </Td>
                    </Tr>

...
describe('Part 1 - Public room listing', () => {
  ...
  it('updates the page with all towns stored in currentPublicTowns', async () => {
    ...
    const renderData = render(wrappedTownSelection());
    await waitFor(() => {
      expectedTowns1.towns.map(town => expect(renderData.getByText(town.friendlyName)).toBeInTheDocument());
    });
    mocklistTowns.mockImplementation(() => listTowns(suffix2));
    jest.advanceTimersByTime(2000);
    await waitFor(() => {
      expectedTowns2.towns.forEach(town => expect(renderData.getByText(town.friendlyName)).toBeInTheDocument());
      expectedTowns1.towns.forEach(town => expect(renderData.queryByText(town.friendlyName)).not.toBeInTheDocument());
    });
  });
  ...
  it('sorts towns by occupancy descending', async () => {
    ...
    for (let i = 1; i < rows.length; i += 1) {
      ...
      for (let j = 0; j < expectedTowns1.towns.length; j += 1) {
        if (j !== i - 1) {
          expect(within(rows[i]).queryByText(expectedTowns1.towns[j].friendlyName)).not.toBeInTheDocument();
        }
      }
    }
    ...
    mocklistTowns.mockImplementation(() => listTowns(suffix2));
    jest.advanceTimersByTime(2000);
    await waitFor(() => expectedTowns2.towns.map(town => expect(renderData.getByText(town.friendlyName)).toBeInTheDocument()));
    ...
    for (let i = 1; i < rows.length; i += 1) {
      ...
      for (let j = 0; j < expectedTowns2.towns.length; j += 1) {
        if (j !== i - 1) {
          expect(within(rows[i]).queryByText(expectedTowns2.towns[j].friendlyName)).not.toBeInTheDocument();
        }
      }
    }
  });
  it('represents each row in the table as specified', async () => {
    ...
    const renderData = render(wrappedTownSelection());
    await waitFor(() => {
      expectedTowns.towns.forEach(town => expect(renderData.getByText(town.friendlyName)).toBeInTheDocument());
    });
    ...
    expectedTowns.towns.forEach(town => {
      ...
      if (row) {
        ...
        expect(within(cells[2]).queryByText(`${town.currentOccupancy}/${town.maximumOccupancy}`)).toBeInTheDocument();
      } else {
